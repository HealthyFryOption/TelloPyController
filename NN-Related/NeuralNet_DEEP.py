...
class NeuralNet(nn.Module):
    ...
    def __init__(self):
        ...
        # Network will take in 1 channel, which means GreyScalled image only. a 3x3 Kernel is used
        # for large images. Even numbers are to be avoided
    ...
    def convolution(self, X):
        # max pooling size of 2x2 and stride value of 2 
    ...
    def forward(self, X):
        ...
        X = X.view(-1, self.toLinearSize) # reshape it to be flattened in the right way
        
        # pass flatten to first connected layer
        X = F.relu(self.fc1(X)) 
        X = F.relu(self.fc2(X))

        # pass first Linear layor output to second/final connected layer.
        X = self.fc3(X) 
